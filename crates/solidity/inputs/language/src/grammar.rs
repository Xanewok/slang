...
use codegen_language_definition::model::{self, Spanned};
    ...
    fn from_dsl_v2() -> Rc<Grammar> {
        ...
        for section in lang.sections {
            for topic in section.topics {
                ...
                for item in topic.items {
                    items.insert(
                        item.name().deref().clone(),
                        (ctx.clone().map(Spanned::into_value), Rc::clone(&item)),
                    );
                }
            }
        }
            ...
            (
                ...
                Rc::new(model::Item::Struct {
                    item: model::StructItem {
                        name: Spanned::without_span(Identifier::from("SourceUnit")),
                        enabled: None,
                        error_recovery: None,
                        fields: IndexMap::from_iter([
                            (
                                Spanned::without_span(Identifier::from("members")),
                                model::Field::Optional {
                                    kind: model::FieldKind::NonTerminal {
                                        item: Spanned::without_span(Identifier::from(
                                            "SourceUnitMembers",
                                        )),
                                    },
                                    enabled: None,
                            ...
                            (
                                Spanned::without_span(Identifier::from("eof_trivia")),
                                model::Field::Optional {
                                    kind: model::FieldKind::NonTerminal {
                                        item: Spanned::without_span(Identifier::from(
                                            "EndOfFileTrivia",
        ...
        for (lex_ctx, mut keywords) in keywords_per_ctxt {
            ...
            let all_keywords = model::EnumItem {
                name: Spanned::without_span(parser_name.clone()),
                enabled: None,
                variants: keywords
                    .iter()
                    .map(|&ident| model::EnumVariant {
                        name: Spanned::without_span(Identifier::from("unused")),
                        enabled: None,
                        reference: Spanned::without_span(ident.clone()),
                    })
                    .collect(),
            };
        ...
        Rc::new(Grammar {
            ...
            versions: BTreeSet::from_iter(lang.versions.into_iter().map(Spanned::into_value)),
...
fn resolve_scanner(scanner: model::Scanner, ctx: &mut ResolveCtx) -> ScannerDefinitionNode {
    match scanner {
        ...
        model::Scanner::Not { chars } => {
            ScannerDefinitionNode::NoneOf(chars.into_iter().map(|x| *x).collect())
        }
        ...
        model::Scanner::Atom { atom } => ScannerDefinitionNode::Literal(atom.into_value()),
...
fn resolve_fragment(fragment: model::FragmentItem, ctx: &mut ResolveCtx) -> ScannerDefinitionNode {
    resolve_scanner(fragment.scanner, ctx).versioned(fragment.enabled.map(Spanned::into_value))
}
...
fn resolve_token(token: model::TokenItem, ctx: &mut ResolveCtx) -> ScannerDefinitionNode {
    let resolved_defs: Vec<_> = token
        ...
        .into_iter()
        .map(|def| {
            resolve_scanner(def.scanner, ctx).versioned(def.enabled.map(Spanned::into_value))
        })
        .collect();
...
fn resolve_keyword(keyword: model::KeywordItem) -> ScannerDefinitionNode {
    ...
    let defs: Vec<_> = keyword
        ...
        .map(|def| {
            ...
            match (
                def.enabled.map(Spanned::into_value),
                def.reserved.map(Spanned::into_value),
            ) {
...
fn resolve_keyword_value(value: model::KeywordValue) -> ScannerDefinitionNode {
    match value {
        ...
        model::KeywordValue::Atom { atom } => ScannerDefinitionNode::Literal(atom.into_value()),
    }
}
...
fn resolve_field(field: model::Field, ctx: &mut ResolveCtx) -> ParserDefinitionNode {
    match field {
        ...
        model::Field::Optional { kind, enabled } => ParserDefinitionNode::Optional(Box::new(
            resolve_field_kind(kind, ctx).versioned(enabled.map(Spanned::into_value)),
        )),
    }
}
...
fn resolve_sequence_like(
    ...
    fields: IndexMap<Spanned<Identifier>, model::Field>,
    ...
    let variants = item
        ...
        .map(|variant| {
            resolve_grammar_element(&variant.reference, ctx)
                ...
                .versioned(variant.enabled.map(Spanned::into_value))
        })
        .collect();

    ParserDefinitionNode::Choice(variants).versioned(item.enabled.map(Spanned::into_value))
}
...
fn resolve_repeated(item: model::RepeatedItem, ctx: &mut ResolveCtx) -> ParserDefinitionNode {
    ...
    let repeated = if item.allow_empty.map(Spanned::into_value).unwrap_or(false) {
        ParserDefinitionNode::ZeroOrMore
    } else {
        ParserDefinitionNode::OneOrMore
    };

    repeated(body).versioned(item.enabled.map(Spanned::into_value))
}
...
fn resolve_separated(item: model::SeparatedItem, ctx: &mut ResolveCtx) -> ParserDefinitionNode {
    ...
    let separated_by = ParserDefinitionNode::SeparatedBy(Box::new(body), Box::new(separator))
        .versioned(item.enabled.map(Spanned::into_value));

    if item.allow_empty.map(Spanned::into_value).unwrap_or(false) {
...
fn resolve_precedence(
    ...
    let primaries: Vec<_> = item
        ...
        .map(|prim| {
            resolve_grammar_element(&prim.expression, ctx)
                .into_parser_def_node()
                .versioned(prim.enabled.map(Spanned::into_value))
        })
        .collect();
    ...
    for expr in item.precedence_expressions {
        ...
        for op in &expr.operators {
            operators.push((
                op.enabled
                    ...
                    .map(enabled_to_range)
                    .unwrap_or_default(),
                model_to_enum(op.model.clone().into_value()),
                // TODO: Don't leak
        ...
        let defs: Vec<_> = expr
            ...
            .into_iter()
            .map(|op| {
                resolve_sequence_like(
                    op.enabled.map(Spanned::into_value),
                    op.fields,
                    op.error_recovery,
                    ctx,
                )
        ...
        ctx.resolved
            .insert(name.into_value(), GrammarElement::ParserDefinition(thunk));
    }
